 ...
 */
class MongoDbNotNewCallbackTests {
	...
	void marksEntityAsNotNewAfterLoad() {
		...
		assertThat(callback.onAfterConvert(new SamplePersistable(), null, null).isNew()).isFalse();
	}
	...
	void marksEntityAsNotNewAfterSave() {
		...
		SamplePersistable sample = callback.onAfterSave(new SamplePersistable(), null, null);

		assertThat(sample.isNew()).isFalse();
		assertThat(callback.onAfterSave(sample, null, null).isNew()).isFalse();
	}
}
